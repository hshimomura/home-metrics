...
CREATE TABLE IF NOT EXISTS sensor_alert_rules (
    ...
    for_duration interval NOT NULL DEFAULT interval '0 seconds' CHECK (for_duration >= interval '0 seconds'),
    max_data_age interval NOT NULL DEFAULT interval '10 minutes' CHECK (max_data_age > interval '0 seconds'),
    severity text NOT NULL DEFAULT 'warning' CHECK (severity IN ('info', 'warning', 'critical')),
    enabled boolean NOT NULL DEFAULT true,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CHECK (
        (direction = 'above' AND clear_threshold < trigger_threshold)
        OR (direction = 'below' AND clear_threshold > trigger_threshold)
    )
);

CREATE INDEX IF NOT EXISTS sensor_alert_rules_enabled_device_idx
    ON sensor_alert_rules (enabled, mac, metric);

CREATE TABLE IF NOT EXISTS sensor_alert_states (
    alert_rule_id bigint PRIMARY KEY REFERENCES sensor_alert_rules(id) ON DELETE CASCADE,
    status text NOT NULL DEFAULT 'normal' CHECK (status IN ('normal', 'pending', 'firing')),
    ...
    evaluation_status text NOT NULL DEFAULT 'no_data' CHECK (evaluation_status IN ('ok', 'stale', 'no_data', 'disabled')),
    updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS sensor_alert_states_status_updated_idx
    ON sensor_alert_states (status, updated_at DESC);

CREATE TABLE IF NOT EXISTS sensor_alert_events (
    id bigserial PRIMARY KEY,
    alert_rule_id bigint REFERENCES sensor_alert_rules(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS sensor_alert_events_occurred_idx
    ON sensor_alert_events (occurred_at DESC);

CREATE INDEX IF NOT EXISTS sensor_alert_events_mac_occurred_idx
    ON sensor_alert_events (mac, occurred_at DESC);
