...
sensor device を maintenance mode に入れる、または解除します。maintenance mode は
sensor freshness alert の評価対象から外すための運用状態です。collector alert や energy alert
には使いません。
...
管理画面では sensor alert の行だけに iOS 風の maintenance toggle を表示します。
